...
def generate_sparse_diagonally_dominant_system(n, density=0.01, seed=None):
    ...
    A_rand = sp.random(n, n, density=density, format='csr', random_state=seed, dtype=np.float64)
    A_rand.eliminate_zeros()

    A_diag_abs_row_sum = np.array(np.abs(A_rand).sum(axis=1)).flatten()
    
    new_diag_values = A_diag_abs_row_sum - np.abs(A_rand.diagonal()) + np.random.uniform(0.1, 1.0, n)
    ...
    print(f"Diagonally dominant matrix A ({A.shape}) generated with {A.nnz} non-zero elements.")
...
def generate_sparse_non_diagonally_dominant_system(n, density=0.01, seed=None):
    ...
    A_diag_abs_row_sum = np.array(np.abs(A_rand).sum(axis=1)).flatten()
    new_diag_values = A_diag_abs_row_sum - np.abs(A_rand.diagonal()) - np.random.uniform(0.1, 1.0, n)
    new_diag_values[new_diag_values <= 0] = -1.0  # Ensure diagonal values are smaller than off-diagonal sums
    ...
    print(f"Non-diagonally dominant matrix A ({A.shape}) generated with {A.nnz} non-zero elements.")
    return A, b, x_true

import numpy as np
import scipy.sparse as sp

def generate_sparse_large_spectral_radius_system(n, density=0.01, scale_offdiag=5.0, seed=None, check_spectral_radius=False):
    """
    ...
    print(f"Sparse matrix with potentially large spectral radius generated. Shape: {A.shape}, nnz: {A.nnz}")
...
def calculate_relative_residual_norm(A, x, b):
    ...
    try:
        ...
        print(f"Error calculating residual for x of shape {x.shape if x is not None else 'None'}: {e}")
        return np.nan

def calculate_solution_error(x_computed, x_true):
    if x_computed is None or x_true is None or \
       np.any(np.isnan(x_computed)) or np.any(np.isinf(x_computed)):
        return np.nan
...
if __name__ == "__main__":
    ...
    A_sparse, b_np, x_true_np = generate_sparse_diagonally_dominant_system(N, density=DENSITY, seed=SEED)
    ...
    print(f"Norm of b: {np.linalg.norm(b_np):.2e}, Norm of x_true: {np.linalg.norm(x_true_np):.2e}")
    ...
    x_spsolve, time_spsolve, _ = solve_scipy_spsolve_baseline(A_sparse.copy(), b_np.copy())
    if x_spsolve is not None:
        ...
        results['Baseline spsolve'] = {'time': time_spsolve, 'rel_res': rel_res_spsolve, 'sol_err': err_spsolve, 'iters': 'N/A'}
    else:
        print(f"SciPy spsolve (baseline) failed. Time: {time_spsolve:.4f} s")
        results['Baseline spsolve'] = {'time': time_spsolve, 'rel_res': np.nan, 'sol_err': np.nan, 'iters': 'N/A'}
    print("-" * 70)
    ...
    if x_splu_scipy is not None:
        rel_res_splu_scipy = calculate_relative_residual_norm(A_sparse, x_splu_scipy, b_np)
        ...
        results['Baseline splu'] = {'time': time_splu_scipy, 'rel_res': rel_res_splu_scipy, 'sol_err': err_splu_scipy, 'iters': 'N/A'}
    else:
        print(f"SciPy splu (baseline) failed. Time: {time_splu_scipy:.4f} s")
        results['Baseline splu'] = {'time': time_splu_scipy, 'rel_res': np.nan, 'sol_err': np.nan, 'iters': 'N/A'}
    print("-" * 70)
    ...
    if x_mge is not None:
        ...
        results['Manual GE (Method 1)'] = {'time': time_mge, 'rel_res': rel_res_mge, 'sol_err': err_mge, 'iters': 'N/A'}
    else:
        print(f"Manual GE (Method 1) failed or was skipped. Time: {time_mge:.4f} s")
        results['Manual GE (Method 1)'] = {'time': time_mge, 'rel_res': np.nan, 'sol_err': np.nan, 'iters': 'N/A'}
    print("-" * 70)

    # Method 2: Gauss-Seidel
    print("\n--- Method 2: Gauss-Seidel Iteration ---")
    gs_max_iters = 5000 if N <= 1000 else (2000 if N <=10000 else 1000) # More iters for smaller, fewer for huge
    gs_tol = 1e-8 # Tighter tolerance
    x_gs, time_gs, iters_gs = solve_gauss_seidel(A_sparse.copy(), b_np.copy(), max_iters=gs_max_iters, tol=gs_tol)
    if x_gs is not None and not (np.any(np.isnan(x_gs)) or np.any(np.isinf(x_gs))):
        ...
        results['Gauss-Seidel (Method 2)'] = {'time': time_gs, 'rel_res': rel_res_gs, 'sol_err': err_gs, 'iters': iters_gs}
    else:
        print(f"Gauss-Seidel failed or produced invalid solution. Time: {time_gs:.4f} s, Iterations: {iters_gs}")
        results['Gauss-Seidel (Method 2)'] = {'time': time_gs, 'rel_res': np.nan, 'sol_err': np.nan, 'iters': iters_gs}
    print("-" * 70)
